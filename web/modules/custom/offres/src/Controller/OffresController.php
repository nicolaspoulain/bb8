...
class OffresController extends ControllerBase {
  ...
  public function list() {
    ...
    'prio_nat'     => array('data' => t('Priorité Nat.'), 'class' => 'jaune'),
    'prio_aca'     => array('data' => t('Priorité Aca.'), 'class' => 'jaune'),
  ...
  foreach ($result = $pager->execute()->fetchAll() as $r) {
    // $hp = render(drupal_get_form('gbb_offres_nb_hp_form', // $r->nb_hp, $r->co_omodu)) ;
    // $vac = render(drupal_get_form('gbb_offres_nb_vac_form', // $r->nb_vac, $r->co_omodu)) ;
    // $taux = render(drupal_get_form('gbb_offres_taux_form', // $r->taux, $r->co_omodu)) ;
    // $ht2 = render(drupal_get_form('gbb_offres_ht2_form', // $r->ht2, $r->co_omodu)) ;
    // $prio_nat = render(drupal_get_form('gbb_offres_prio_nat_form', // $r->prio_nat, $r->co_omodu)) ;
    // $prio_aca = render(drupal_get_form('gbb_offres_prio_aca_form', // $r->prio_aca, $r->co_omodu)) ;
    // $offre_new = render(drupal_get_form('gbb_offres_offre_new_form', // $r->offre_new, $r->co_omodu)) ;
    ...
    // $catOffre = render(drupal_get_form('gbb_offres_categories_form', // $r->offre_cat, $r->co_omodu)) ;
    // $interdisc = render(drupal_get_form('gbb_offres_interdisc_form', // $r->interdisc, $r->co_omodu)) ;
    // $iufm = render(drupal_get_form('gbb_offres_iufm_form', // $r->iufm, $r->co_omodu)) ;
    // $nouv_offreur = render(drupal_get_form('gbb_offres_nouv_offreur_form', // $r->nouv_offreur, $r->co_omodu)) ;
    // $pub_des = render(drupal_get_form('gbb_offres_pub_des_form', // $r->pub_des, $r->co_omodu)) ;
    // $anim_peda = render(drupal_get_form('gbb_offres_anim_peda_form', // $r->anim_peda, $r->co_omodu)) ;
    // $foad= render(drupal_get_form('gbb_offres_foad_form', // $r->foad, $r->co_omodu)) ;
