...
/**
 * @file
 * Contains Drupal\offres\Form\PtrimForm.
 */
...
/**
 * Implements the PtrimForm form controller.
 */
class PtrimForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getTitle() {
    return 'Modification de la ptrim';
  }
   ...
   * {@inheritdoc}
   */
  public function buildForm(array $form, FormStateInterface $form_state, $ptrim=NULL, $co_omodu=NULL) {

  $form['co_omodu']  = array('#type' => 'hidden','#value' => $co_omodu );
  $form['ptrim'] = array(
    '#type' => 'checkbox', '#size' => 1,
    '#default_value' => (isset($ptrim))? (int)$ptrim : '0',
    '#ajax' => array(
  ...
  public function saveAjax(array &$form, FormStateInterface $form_state) {
    ...
    $entry = array(
      'ptrim'  => $form_state->getValue('ptrim'),
    );
