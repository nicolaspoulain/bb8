...
/**
 * @file
 * Contains Drupal\offres\Form\EchangeForm.
 */
...
/**
 * Implements the EchangeForm form controller.
 */
class EchangeForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getTitle() {
    return 'Modification de la echange';
  }
   ...
   * {@inheritdoc}
   */
  public function buildForm(array $form, FormStateInterface $form_state, $echange=NULL, $co_omodu=NULL) {

  $form['co_omodu']  = array('#type' => 'hidden','#value' => $co_omodu );
  $form['echange'] = array(
    '#type' => 'checkbox', '#size' => 1,
    '#default_value' => (isset($echange))? (int)$echange : '0',
    '#ajax' => array(
  ...
  public function saveAjax(array &$form, FormStateInterface $form_state) {
    ...
    $entry = array(
      'echange'  => $form_state->getValue('echange'),
    );
